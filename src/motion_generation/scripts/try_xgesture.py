...
def main(config):
    ...
    print("Loading model...")
    model = XGestureTransformer.load( config['gesture_transformer_path'] / "best_model.pt", text_embedding=text_embedding, device=device)
    model.eval()
    print("Model loaded.")
    ...
    look_ahead = model.look_ahead_steps
    for i in bar:
        ...
        logits, attn = model(
            src=encoder_input[:,:i+look_ahead],
            tgt=decoder_input,
    ...
    all_true = torch.ones_like(mask, dtype=torch.bool)
...
if __name__ == "__main__":
    ...
    config = {
        ...
        'gesture_transformer_path': PRJ_ROOT / 'weights/transformer/x_drop/',
